                ...
                manager.loadItemOrdered(name, items.get(i), new AudioLoadResultHandler() 
                {
                    ...
                    public void trackLoaded(AudioTrack at) 
                    {
                        if(config.isTooLong(at))
                            errors.add(new PlaylistLoadError(index, items.get(index), "Cette musique est plus longue que le maximum autorisé"));
                        else
                        {
                            at.setUserData(0L);
                            tracks.add(at);
                    ...
                    {
                        errors.add(new PlaylistLoadError(index, items.get(index), "Aucune musiques trouvées."));
                        done();
                    }
                    ...
                    {
                        errors.add(new PlaylistLoadError(index, items.get(index), "Echec du chargement de la musique: "+fe.getLocalizedMessage()));
                        done();
                    }
                });
            }
